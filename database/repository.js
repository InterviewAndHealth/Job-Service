...
class Repository {
  ...
  async getFilteredJobs(
    ...
    if (jobTitle) {
      filters.push(
        `LOWER(j.job_title) LIKE '%' || LOWER($${values.length + 1}) || '%'`
      );
      values.push(jobTitle);
    }
    if (jobExperience) {
      filters.push(
        `LOWER(j.job_experience) LIKE '%' || LOWER($${
          values.length + 1
        }) || '%'`
      );
      values.push(jobExperience);
    ...
    if (jobLocations) {
      ...
      EXISTS (
        ...
        WHERE LOWER(loc) = ANY(ARRAY(SELECT LOWER(val) FROM UNNEST($${
          values.length + 1
        }::TEXT[]) val))
      )
    `);
    ...
    if (salaryMin) {
      filters.push(
        `CAST(j.salary_min AS NUMERIC) >= CAST($${
          values.length + 1
        } AS NUMERIC)`
      );
      values.push(salaryMin);
    ...
    if (requiredSkills) {
      ...
      EXISTS (
        ...
        WHERE LOWER(skill) = ANY(ARRAY(SELECT LOWER(val) FROM UNNEST($${
          values.length + 1
        }::TEXT[]) val))
      )
    `);
    ...
    if (jobKeywords) {
      filters.push(`
      (
        LOWER(j.job_title) LIKE ANY(ARRAY(SELECT '%' || LOWER(keyword) || '%' FROM UNNEST($${
          values.length + 1
        }::TEXT[]) keyword))
        OR LOWER(j.company_name) LIKE ANY(ARRAY(SELECT '%' || LOWER(keyword) || '%' FROM UNNEST($${
          values.length + 1
        }::TEXT[]) keyword)) 
        OR EXISTS (
          SELECT 1 FROM UNNEST(j.required_skills) skill 
          WHERE LOWER(skill) LIKE ANY(ARRAY(SELECT '%' || LOWER(keyword) || '%' FROM UNNEST($${
            values.length + 1
          }::TEXT[]) keyword))
        )
      )
...
`,
  ...
  async updatestudentscandetails(
    job_id,
    resume_id,
  ...
  async getTalentPoolEntryByInterviewId(interview_id) {
    const result = await DB.query({
  ...
  async moveRecommendedToApplication({
    job_id,
    resume_id,
    candidate_name,
    candidate_email,
  ...
  async getRecruiterIdByJobId(job_id) {
    const result = await DB.query({
      text: "SELECT user_id FROM jobs WHERE job_id = $1",
      values: [job_id],
    });
    return result.rows[0];
  }
